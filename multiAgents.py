...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        if len(foodDists) != 0:
          nearestFoodDist = min(foodDists)
        else:
          nearestFoodDist = 0
        nearestGhostDist = min([manhattanDistance(newPos, ghostPos) for ghostPos in successorGameState.getGhostPositions()])
        return successorGameState.getScore() - nearestFoodDist + nearestGhostDist
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def minimaxSearch(state, agentIndex, depth):
          ...
          if len(moves) == 0:
            return self.evaluationFunction(state)
          next = (minimaxSearch(state.generateSuccessor(agentIndex, m), agentIndex + 1, depth) for m in moves)
          if(agentIndex == 0):
            return max(next)
          else:
            return min(next)
        return max(gameState.getLegalActions(0), key=lambda x: minimaxSearch(gameState.generateSuccessor(0, x), 1, 1))
    ...
    def getAction(self, gameState):
        ...
        def expectimaxSearch(state, agentIndex, depth):
          ...
          if(agentIndex == 0):
            ...
            l = list(next)
                    return sum(l) / len(l)
        return max(gameState.getLegalActions(0), key=lambda x: expectimaxSearch(gameState.generateSuccessor(0, x), 1, 1))
